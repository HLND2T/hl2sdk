...
class Function(_GenericDeclaration):
    ...
    def __str__(self):
        ...
        suffix = "%s %s(%s), 0x%02x, %s" % (self.return_type, self.name, self.parameters, self.modifiers, self.body)
        return self._TypeStringHelper(suffix)


class Method(Function):
    def __init__(
        self, start, end, name, in_class, return_type, parameters, modifiers, templated_types, body, namespace
    ):
        Function.__init__(self, start, end, name, return_type, parameters, modifiers, templated_types, body, namespace)
...
class TypeConverter(object):
    ...
    def ToParameters(self, tokens):
        ...
        def AddParameter(end):
            ...
            parameter_type = Type(
                first_token.start, first_token.end, type_name, templated_types, modifiers, reference, pointer, array
            )
...
class AstBuilder(object):
    ...
    def _CreateVariable(
        ...
        var_type = Type(
            pos_token.start, pos_token.end, type_name, templated_types, type_modifiers, reference, pointer, array
        )
    ...
    def _GetClass(self, class_type, visibility, templated_types):
        ...
        if token.token_type == tokenize.SYNTAX:
            if token.name == ";":
                # Forward declaration.
                return class_type(
                    class_token.start, class_token.end, class_name, None, templated_types, None, self.namespace_stack
                )
            if token.name in "*&":
            ...
            if not self._handling_typedef:
                ...
                if token.token_type != tokenize.NAME:
                    ...
                    new_class = class_type(
                        class_token.start, class_token.end, class_name, bases, None, body, self.namespace_stack
                    )
        ...
        return class_type(
            class_token.start, class_token.end, class_name, bases, templated_types, body, self.namespace_stack
        )
