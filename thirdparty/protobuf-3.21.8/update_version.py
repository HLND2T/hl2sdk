...
def UpdateCMake():
    cmake_files = ("cmake/libprotobuf.cmake", "cmake/libprotobuf-lite.cmake", "cmake/libprotoc.cmake")
    for cmake_file in cmake_files:
        RewriteTextFile(
            cmake_file, lambda line: re.sub(r"SOVERSION ([0-9]+)$", "SOVERSION %s" % GetSharedObjectVersion()[0], line)
        )
...
def UpdateConfigure():
    RewriteTextFile(
        ...
        lambda line: re.sub(
            ...
            ("AC_INIT([Protocol Buffers],[%s],[protobuf@googlegroups.com],[protobuf])" % GetFullVersion()),
            line,
        ),
    )
...
def UpdateCpp():
    cpp_version = "%d%03d%03d" % (NEW_VERSION_INFO[0], NEW_VERSION_INFO[1], NEW_VERSION_INFO[2])
    version_suffix = ""
    if RC_VERSION != -1:
        version_suffix = "-rc%s" % RC_VERSION

    def RewriteCommon(line):
        line = re.sub(r"^#define GOOGLE_PROTOBUF_VERSION .*$", "#define GOOGLE_PROTOBUF_VERSION %s" % cpp_version, line)
        line = re.sub(r"^#define PROTOBUF_VERSION .*$", "#define PROTOBUF_VERSION %s" % cpp_version, line)
        line = re.sub(
        ...
        line = re.sub(
            r"^#define PROTOBUF_VERSION_SUFFIX .*$", '#define PROTOBUF_VERSION_SUFFIX "%s"' % version_suffix, line
        )
    ...
    def RewritePbH(line):
        line = re.sub(r"^#if PROTOBUF_VERSION < .*$", "#if PROTOBUF_VERSION < %s" % cpp_version, line)
        line = re.sub(
            r"^#if .* < PROTOBUF_MIN_PROTOC_VERSION$", "#if %s < PROTOBUF_MIN_PROTOC_VERSION" % cpp_version, line
        )
        return line
...
def UpdateCsharp():
    RewriteXml(
        ...
        lambda document: ReplaceText(
            Find(Find(document.documentElement, "PropertyGroup"), "VersionPrefix"), GetFullVersion(rc_suffix="-rc")
        ),
    ...
    RewriteXml(
        ...
        lambda document: ReplaceText(
            Find(Find(document.documentElement, "metadata"), "version"), GetFullVersion(rc_suffix="-rc")
        ),
    )
...
def UpdateJava():
    RewriteXml(
        "java/pom.xml", lambda document: ReplaceText(Find(document.documentElement, "version"), GetFullVersion())
    )

    RewriteXml(
        "java/bom/pom.xml", lambda document: ReplaceText(Find(document.documentElement, "version"), GetFullVersion())
    )

    RewriteXml(
        "java/core/pom.xml",
        lambda document: ReplaceText(Find(Find(document.documentElement, "parent"), "version"), GetFullVersion()),
    )
    ...
    RewriteXml(
        ...
        lambda document: ReplaceText(Find(document.documentElement, "version"), GetFullVersion()),
    )

    RewriteTextFile(
        "java/README.md",
        lambda line: re.sub(r"<version>.*</version>", "<version>%s</version>" % GetFullVersion(), line),
    )
    ...
    RewriteTextFile(
        "java/lite.md", lambda line: re.sub(r"<version>.*</version>", "<version>%s</version>" % GetFullVersion(), line)
    )
...
def UpdateMakefile():
    RewriteTextFile(
        "src/Makefile.am",
        lambda line: re.sub(
            r"^PROTOBUF_VERSION = .*$", "PROTOBUF_VERSION = %s" % ":".join(map(str, GetSharedObjectVersion())), line
        ),
    )
...
def UpdateObjectiveC():
    RewriteTextFile(
        "Protobuf.podspec",
        lambda line: re.sub(r"^  s.version  = '.*'$", "  s.version  = '%s'" % GetFullVersion(rc_suffix="-rc"), line),
    )
    RewriteTextFile(
        "Protobuf-C++.podspec",
        lambda line: re.sub(r"^  s.version  = '.*'$", "  s.version  = '%s'" % GetFullVersion(rc_suffix="-rc"), line),
...
def UpdatePhp():
    def Callback(document):
        ...
        ReplaceText(Find(stability, "release"), "stable" if RC_VERSION < 0 else "beta")
        ...
        for old_version in changelog.getElementsByTagName("version"):
            if Find(old_version, "release").firstChild.nodeValue == NEW_VERSION:
                print("[WARNING] Version %s already exists in the change log." % NEW_VERSION)
                return
        if RC_VERSION != 0:
            ...
            release = CreateNode(
                ...
                2,
                [
                    CreateNode("version", 3, [FindAndClone(version, "release"), FindAndClone(version, "api")]),
                    CreateNode("stability", 3, [FindAndClone(stability, "release"), FindAndClone(stability, "api")]),
...
def UpdatePython():
    RewriteTextFile(
        ...
        lambda line: re.sub(r"^__version__ = '.*'$", "__version__ = '%s'" % GetFullVersion(rc_suffix="rc"), line),
    )


def UpdateRuby():
    RewriteXml(
        "ruby/pom.xml", lambda document: ReplaceText(Find(document.documentElement, "version"), GetFullVersion())
    )
    RewriteXml(
        "ruby/pom.xml",
        lambda document: ReplaceText(
            Find(Find(Find(document.documentElement, "dependencies"), "dependency"), "version"), GetFullVersion()
        ),
    )
    RewriteTextFile(
        "ruby/google-protobuf.gemspec",
        lambda line: re.sub(
            r'^  s.version     = ".*"$', '  s.version     = "%s"' % GetFullVersion(rc_suffix=".rc."), line
        ),
    )
...
def UpdateBazel():
    RewriteTextFile(
        ...
        lambda line: re.sub(r"^PROTOBUF_VERSION = '.*'$", "PROTOBUF_VERSION = '%s'" % GetFullVersion(), line),
    )
