...
parser.add_argument("data_files", metavar="dataFile", nargs="+", help="testing data files.")
parser.add_argument(
    "--json", action="store_const", dest="json", const="yes", default="no", help="Whether to output json results"
)
parser.add_argument(
    "--behavior_prefix", dest="behavior_prefix", help="The output json format's behavior's name's prefix", default=""
)
...
def run_one_test(filename):
    ...
    benchmark_util = Benchmark(
        full_iteration=len(benchmark_dataset.payload),
        module="py_benchmark",
        setup_method="init",
    ...
    result["benchmarks"][args.behavior_prefix + "_parse_from_benchmark"] = benchmark_util.run_benchmark(
        setup_method_args='"%s"' % (filename)
    )
    benchmark_util.set_test_method("serialize_to_benchmark")
    result["benchmarks"][args.behavior_prefix + "_serialize_to_benchmark"] = benchmark_util.run_benchmark(
        setup_method_args='"%s"' % (filename)
    )
    return result
...
class Benchmark:
    ...
    def dry_run(self, test_method_args="", setup_method_args=""):
        return timeit.timeit(
            stmt="%s(%s)" % (self.test_method, test_method_args),
...
if __name__ == "__main__":
    ...
    if args.json != "no":
        ...
        for result in results:
            print("Message %s of dataset file %s" % (result["message_name"], result["filename"]))
            print(
