...
_NOW = "%d%02d%02d" % (datetime.datetime.now().year, datetime.datetime.now().month, datetime.datetime.now().day)
...
def upload_result(result_list, metadata):
    for result in result_list:
        ...
        if not big_query_utils.insert_rows(bq, _PROJECT_ID, _DATASET, _TABLE + "$" + _NOW, [row]):
...
if __name__ == "__main__":
    ...
    upload_result(
        result_parser.get_result_from_file(
